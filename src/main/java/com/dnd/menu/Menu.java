...
import com.dnd.game.CharacterOutOfBoundsException;
...
/**
 * Text-based console menu driving the whole application: choosing or creating a
 * hero, editing it, and playing the board game while persisting changes.
 */
public final class Menu {
    ...
    /**
     * @param heroRepository the repository used to load and persist heroes
     */
    ...
    private void playWithExistingHero(Scanner scanner, Game game) {
        ...
        for (int i = 0; i < heroes.size(); i++) {
            ...
            System.out.printf("%d) #%d %s (%s) HP=%d Attack=%d%n",
                    i + 1, h.id(), h.name(), h.type(), h.lifePoints(), h.baseAttack());
        }
    ...
    private void playGame(Scanner scanner, Game game, HeroSession session) {
        ...
        System.out.println("Current tile: " + game.getCurrentTile());
        ...
        while (game.isRunning()) {
            ...
            try {
                ...
                System.out.printf("You rolled %d. You are now on tile %d/%d.%n",
                        outcome.roll(), outcome.position(), game.getBoardSize());
                System.out.println("You landed on: " + outcome.landedTile());

            } catch (CharacterOutOfBoundsException ex) {
                System.out.printf("Roll would move you out of the board (%d -> %d). You stay on tile %d/%d.%n",
                        ex.getCurrentPosition(), ex.getAttemptedPosition(),
                        ex.getCurrentPosition(), ex.getBoardSize());
            }
        }
    ...
    private void printMainMenu() {
        ...
        System.out.println("1) Choose existing hero");
    ...
    private CharacterType readCharacterType(Scanner scanner) {
        while (true) {
            ...
            int choice = readInt(scanner, "Your choice: ");
            switch (choice) {
                case 1 -> { return CharacterType.WARRIOR; }
                case 2 -> { return CharacterType.WIZARD; }
